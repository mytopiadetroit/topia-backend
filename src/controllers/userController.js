...
module.exports.getLoginStatsRange = async (req, res) => {
  try {
    ...
    if (range === 'monthly') {
      ...
      const map = new Map(raw.map((r) => [ `${r._id.y}-${r._id.m}-${r._id.d}`, r.count ]));
      ...
      return res.status(200).json({ success: true, today, range: 'monthly', series: result, total });
    }
    ...
    if (range === 'yearly') {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        months.push({ y: d.getFullYear(), m: d.getMonth() + 1, date: d, count: 0 });
      }
      ...
      return res.status(200).json({ success: true, thisMonth, range: 'yearly', series: result, total });
    }
